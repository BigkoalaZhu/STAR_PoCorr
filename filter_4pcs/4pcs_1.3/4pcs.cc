...
namespace match_4pcs {
	...
	namespace {
		...
		double ComputeRigidTransformation(vector<pair<Point3D, Point3D> >* pairs, cv::Mat* rotation, cv::Point3f* translation, cv::Point3f* center) 
		{
			if (pairs->size() == 0|| rotation == NULL || translation == NULL ||center == NULL)
				return kLargeNumber;
			...
			*translation = centroid1 - centroid2;
			...
			for (int i = 0; i < 3; ++i) {
				...
				rms += sqrt(
					Square(
					transformed.at<double>(0, 0) - ((*pairs)[i].first.x - centroid1.x))
					+ Square(
		...
		void Transform(const cv::Mat &rotation, const cv::Point3f& center, const cv::Point3f& translate, Point3D* point) 
		{
				if (point == NULL)
					return;
				cv::Mat first(3, 1, CV_64F), transformed;
		...
		float distSegmentToSegment(const cv::Point3f& p1, const cv::Point3f& p2, const cv::Point3f& q1, const cv::Point3f& q2, double* invariant1, double* invariant2) 
		{
	...
	private:
		...
		// Sampled subset of P (indices).
		std::vector<int> sampled_P_;
		// Sampled subset of P (indices).
		std::vector<int> sampled_Q_;
	...
	bool Match4PCSImpl::TryQuadrilateral(double* invariant1, double* invariant2) {
		if (invariant1 == NULL || invariant2 == NULL)
			return false;
		...
		for (int i = 0; i < 4; ++i) {
			for (int j = 0; j < 4; ++j) {
				...
				float segment_distance = distSegmentToSegment(base_3D_[i], base_3D_[j],
					base_3D_[k], base_3D_[l],
					&local_invariant1,
					&local_invariant2);
	...
	bool Match4PCSImpl::SelectQuadrilateral(double *invariant1, double *invariant2,
		...
		int *base4) {
			if (invariant1 == NULL || invariant2 == NULL || base1 == NULL || base2 == NULL
				|| base3 == NULL || base4 == NULL)
				return false;
					...
					for (int i = 0; i < sampled_P_3D_.size(); ++i) {
						...
						if (d1 >= too_small && d2 >= too_small && d3 >= too_small) {
							...
							double distance = fabs(
								A * sampled_P_3D_[i].x + B * sampled_P_3D_[i].y
	...
	double Match4PCSImpl::Verify(const cv::Mat& rotation, const cv::Point3f& center,
		const cv::Point3f& translate) {
			...
			float epsilon = options_.max_distance;
			int good_points = 0;
			...
			const float cos_dist = cos(min(90.0, options_.max_normal_difference) * M_PI / 180.0);
			for (int i = 0; i < number_of_points; ++i) {
				...
				if (sqrt(distances[0]) < epsilon) {
					...
					if (cv::norm(p.rgb() - q.rgb()) < options_.max_color_distance &&
	...
	void Match4PCSImpl::BruteForcePairs(double pair_distance, double pair_normals_angle,
		double pair_distance_epsilon, int base_point1,
		int base_point2,
		vector<pair<int, int> >* pairs) {
			...
			const float norm_threshold = 0.5 * options_.max_normal_difference * M_PI / 180.0;
			...
			for (int j = 0; j < sampled_Q_3D_.size(); ++j) {
				...
				for (int i = j + 1; i < sampled_Q_3D_.size(); ++i) {
					...
					float distance = cv::norm(q - p);
					double first_normal_angle = cv::norm(q.normal() - p.normal());
					double second_normal_angle = cv::norm(q.normal() + p.normal());
					// Take the smaller normal distance.
					float first_norm_distance = min(
						fabs(first_normal_angle - pair_normals_angle),
						fabs(second_normal_angle - pair_normals_angle));
					// Verify appropriate angle between normals and distance.
					if (fabs(distance - pair_distance) < pair_distance_epsilon
						&& (first_norm_distance < norm_threshold)) {
							...
							if (acos(segment1.dot(segment2)) <= options_.max_angle * M_PI / 180.0
								&& cv::norm(p - base_3D_[base_point1]) < options_.max_translation_distance
								&& cv::norm(q - base_3D_[base_point2]) < options_.max_translation_distance
								&& cv::norm(p.rgb() - base_3D_[base_point1].rgb()) < options_.max_color_distance
							...
							if (acos(segment1.dot(segment2)) <= options_.max_angle * M_PI / 180.0
								&& cv::norm(p - base_3D_[base_point1]) < options_.max_translation_distance
								&& cv::norm(q - base_3D_[base_point2]) < options_.max_translation_distance
								&& cv::norm(q.rgb() - base_3D_[base_point2].rgb()) < options_.max_color_distance
								&& cv::norm(p.rgb() - base_3D_[base_point1].rgb())
								< options_.max_color_distance) {
									// Add ordered pair.
	...
	bool Match4PCSImpl::TryOneBase() {
		...
		BruteForcePairs(distance1, normal_angle1, distance_factor*options_.max_distance, 0, 1, &pairs1);
		BruteForcePairs(distance2, normal_angle2, distance_factor*options_.max_distance, 2, 3, &pairs2);

		if (pairs1.size() == 0 || pairs2.size() == 0) {
			return false;
		}
		if (!FindCongruentQuadrilaterals(invariant1, invariant2, distance_factor*options_.max_distance,
			distance_factor*options_.max_distance, pairs1, pairs2,
			&congruent_quads)) {
				return false;
		}
		//if (congruent_quads.size() > sampled_Q_3D_.size() * 0.5) {
		//  return false;
		//}
		...
		for (int i = 0; i < congruent_quads.size(); ++i) {
			...
			float theta_x = fabs(
				atan2(rotation.at<double>(2, 1), rotation.at<double>(2, 2)));
			float theta_y = fabs(
				atan2(
				-rotation.at<double>(2, 0),
				sqrt(
				Square(rotation.at<double>(2, 1))
				+ Square(rotation.at<double>(2, 2)))));
			float theta_z = fabs(
				atan2(rotation.at<double>(1, 0), rotation.at<double>(0, 0)));
			...
			if (theta_x <= options_.max_angle * M_PI / 180.0
				...
				&& theta_z <= options_.max_angle * M_PI / 180.0) {
					...
					if (f < distance_factor*options_.max_distance) {
	...
	void Match4PCSImpl::Initialize(const std::vector<Point3D>& P,
		const std::vector<Point3D>& Q) {
			...
			int sample_fraction_Q = Q.size() / options_.sample_size;

			// Sample the sets P and Q uniformly.
			for (int i = 0; i < P.size(); ++i) {
				if (rand() % sample_fraction_P == 0) {
					sampled_P_3D_.push_back(P[i]);
					sampled_P_.push_back(i);
				}
			}
			for (int i = 0; i < Q.size(); ++i) {
				if (rand() % sample_fraction_Q == 0) {
					sampled_Q_3D_.push_back(Q[i]);
					sampled_Q_.push_back(i);
				}
			}
			...
			max_base_diameter_ = P_diameter_;

			// RANSAC probability and number of needed trials.
			double first_estimation = log(kSmallError)
				/ log(
				1.0
				- pow(options_.overlap_estimation,
			...
			number_of_trials_ = static_cast<int>(first_estimation
				* (P_diameter_ / kDiameterFraction) / max_base_diameter_);
	...
	bool Match4PCSImpl::Perform_N_steps(int n, cv::Mat* transformation,
		std::vector<Point3D>* Q) {
			...
			for (int i = current_trial_; i < current_trial_ + n; ++i) {
				ok = TryOneBase();
				float fraction_try = static_cast<float>(i) / static_cast<float>(number_of_trials_);
				float fraction_time = static_cast<float>(clock() - t0) / 1000000.0 / 60.0;
				...
				if (ok || i > number_of_trials_ || fraction > 0.99)
					break;
			}
			...
			if (best_LCP_ > last_best_LCP) {
				...
				*Q = Q_copy_;
				...
				transformation->at<double>(0, 3) = centroid_.x - p.x + translate_.x
					+ centroid_P_.x;
				...
				transformation->at<double>(1, 3) = centroid_.y - p.y + translate_.y
					+ centroid_P_.y;
				...
				transformation->at<double>(2, 3) = centroid_.z - p.z + translate_.z
					+ centroid_P_.z;
	...
	float Match4PCSImpl::ComputeTransformation(const std::vector<Point3D>& P,
		...
		cv::Mat* transformation) {
			...
			*transformation = cv::Mat(4, 4, CV_64F, cv::Scalar(0.0));
			for (int i = 0; i < 4; ++i)
				transformation->at<double>(i, i) = 1.0;
	...
	float Match4PCS::ComputeTransformation(const std::vector<Point3D>& P, std::vector<Point3D>* Q, cv::Mat* transformation) {
