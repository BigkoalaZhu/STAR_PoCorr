...
void filter_icp::applyFilter(RichParameterSet* pars) {
    ...
    if(use_normalization) {
		...
		Eigen::AlignedBox3d bbox = m2->bbox();
		Eigen::Vector3d s = bbox.sizes();
    ...
    //////////////////////////////////////////////////
    SurfaceMesh::VertexCoordinatesIterator vci(m2);
    ...
    for(int icp=0; icp<it; showProgress(double(++icp)/it)) {
        ...
        if (draw_correspondences) {
            ...
            for(unsigned int i=0; i<X.cols(); ++i) {
				...
				Eigen::Vector3d p1(X(0,i),  X(1,i), X(2,i));
                Eigen::Vector3d p2(Yp(0,i),Yp(1,i),Yp(2,i));
                if(r(i) > maxdist)
